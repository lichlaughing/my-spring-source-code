...
class CglibAopProxy implements AopProxy, Serializable {
		...
		if (proxySuperClass != Object.class) {
			...
			for (Method method : methods) {
				...
				if (!Modifier.isStatic(mod) && !Modifier.isPrivate(mod)) {
					// 方法不是静态、不是私有的、方法是final修饰的。final的方法是无法代理的
	...
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		...
		// targetSource 是 SingletonTargetSource 也就是单例,true
		...
		Callback[] mainCallbacks = new Callback[]{
				...
				targetDispatcher,
				this.advisedDispatcher, // 自定义增强的方法
				new EqualsInterceptor(this.advised), // equals 方法
				new HashCodeInterceptor(this.advised) // hashcode 方法
		};
