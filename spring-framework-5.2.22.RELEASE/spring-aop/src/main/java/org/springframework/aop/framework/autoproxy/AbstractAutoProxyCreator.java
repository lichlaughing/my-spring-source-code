...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	// 缓存bean的 cacheKey-Class,表示该bean被代理的Class类型
	private final Map<Object, Class<?>> proxyTypes = new ConcurrentHashMap<>(16);

	// 缓存bean的 cacheKey-boolean，表示该bean是否需要代理
	...
	public Object postProcessAfterInitialization(@Nullable Object bean, String beanName) {
		if (bean != null) {
			...
			Object cacheKey = getCacheKey(bean.getClass(), beanName);
			/*
				从earlyProxyReferences 缓存判断
				earlyProxyReferences中缓存的是已经创建好的代理对象
				如果bean不存在相互引用的话，那么这个this.earlyProxyReferences.remove(cacheKey)方法始终返回null
			 */
	...
	protected Object getCacheKey(Class<?> beanClass, @Nullable String beanName) {
		if (StringUtils.hasLength(beanName)) {
			// 判断是否为FactoryBean，是的话返回加上FactoryBean独有的前缀&返回
			// 不是的话直接返回beanName
	...
	protected Object createProxy(Class<?> beanClass, @Nullable String beanName,
		...
		/*
			决定到底是代理对象，还是代理接口。
		 */
		// 判断是否配置了 proxy-target-class="true"，例如： <aop:aspectj-autoproxy proxy-target-class="true"/>
		// true 表示强制使用CGLIB代理对象所有的方法
		if (proxyFactory.isProxyTargetClass()) {
			// 判断当前对象是否是JDK代理生成的，是的话把接口拿过来
			// 当beanClass为Proxy.newProxyInstance方法生成的话，那么返回TRUE
			...
			// 既然没有强制那么判断是否应该使用cglib代理
			if (shouldProxyTargetClass(beanClass, beanName)) {
				proxyFactory.setProxyTargetClass(true);
			} else {
				// 判断下是否能代理接口，也就是能不能用jdk动态代理，不能的话使用cglib代理
