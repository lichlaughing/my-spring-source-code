...
public class ContextLoader {
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		// 判断下，如果Web容器中已经注册了同名的，说明重复初始化了，抛异常
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		...
		// 上下文必须是ConfigurableWebApplicationContext的子类
		if (!ConfigurableWebApplicationContext.class.isAssignableFrom(contextClass)) {
			throw new ApplicationContextException("Custom context class [" + contextClass.getName() +
					"] is not of type [" + ConfigurableWebApplicationContext.class.getName() + "]");
		}
		// 实例化Class，转换成ConfigurableWebApplicationContext返回
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		// 从 servletContext加载的web.xml中读取配置的 context类信息。我记得自己好像没有配置过
		String contextClassName = servletContext.getInitParameter(CONTEXT_CLASS_PARAM);
		if (contextClassName != null) {
			try {
				// 如果contextClassName存在，则利用工具类实例化对象
		...
		// 如果没有配置contextClassName，则从defaultStrategies缓存中拿到默认的WerApplicationContext对应的ClassName
		// 也就是从配置文件 ContextLoader.properties 读取的默认配置,也就是使用的 XmlWebApplicationContext
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		...
		// 2.给上下文设置ServletContext
		wac.setServletContext(sc);
		// 3.从ServletContext中解析配置文件的地址
	...
	protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
		...
		// 4.对实例化的initializerClass排个序
		AnnotationAwareOrderComparator.sort(this.contextInitializers);
		// 5.initializer.initialize 进行自定义操作
	...
	protected List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>>
		...
		List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>> classes =
				new ArrayList<>();
		// 从servletContext解析配置在web.xml的参数globalInitializerClasses
		String globalClassNames = servletContext.getInitParameter(GLOBAL_INITIALIZER_CLASSES_PARAM);
		if (globalClassNames != null) {
			// 如果配置不为空，多个类分割，实例化类实例后加入集合中，注意不是实例化对象
			for (String className : StringUtils.tokenizeToStringArray(globalClassNames, INIT_PARAM_DELIMITERS)) {
				classes.add(loadInitializerClass(className));
			}
		}
		// 从servletContext解析配置在web.xml的参数contextInitializerClasses
